...
import { Download, Database, ArrowUpRight, Settings, Eye, EyeOff, Filter, Map } from 'lucide-react'
...
export default function Home() {
  ...
  const importOrderByName = async () => {
    ...
    setIsLoadingOrders(true)
    try {
      // Clean the order name (remove # if present for the API call)
      const cleanOrderName = orderNameInput.trim().replace('#', '')
      const response = await fetch(`/api/shopify/orders/by-name/${cleanOrderName}`)
      const data = await response.json()
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (orderFilters.financialStatus !== 'all' && order.financial_status !== orderFilters.financialStatus) {
      return false
    }
    
    // Fulfillment status filter
    if (orderFilters.fulfillmentStatus !== 'all') {
      if (orderFilters.fulfillmentStatus === 'fulfilled' && order.fulfillment_status !== 'fulfilled') {
        return false
      }
      if (orderFilters.fulfillmentStatus === 'unfulfilled' && order.fulfillment_status === 'fulfilled') {
        return false
      }
    }
    ...
    if (orderFilters.netsuiteStatus !== 'all') {
      if (orderFilters.netsuiteStatus === 'pushed' && !order.netsuiteDepositNumber) {
        return false
      }
      if (orderFilters.netsuiteStatus === 'not_pushed' && order.netsuiteDepositNumber) {
        return false
      }
    }
  ...
  const filteredPayouts = allPayouts.filter(payout => {
    ...
    if (!filters.netsuiteStatus.all) {
      ...
      if (filters.netsuiteStatus.pushed && !hasNetSuiteId) return false
      if (filters.netsuiteStatus.not_pushed && hasNetSuiteId) return false
    }

    // Payout Status filter
    if (!filters.payoutStatus.all) {
      if (filters.payoutStatus.paid && payout.status !== 'paid') return false
      if (filters.payoutStatus.pending && payout.status !== 'pending') return false
    }
    ...
    switch (activeSection) {
          ...
          <div className="flex h-screen bg-slate-100">
            ...
            <div className="w-64 bg-slate-800 text-white flex flex-col">
              <div className="p-4 border-b border-slate-700">
                <h1 className="text-lg font-semibold">Pirani Sync</h1>
              </div>
              
              <nav className="flex-1 p-4 space-y-2">
                <div className="text-sm text-slate-400 mb-4">Navigation</div>
                <div className="text-sm text-slate-300 mb-2">Dashboard</div>
                <div className="text-sm text-slate-300 mb-2">NetSuite</div>
                <div className="text-sm text-slate-300 mb-2">Shopify</div>
                <div className="ml-4 mb-2">
                  <button className="text-sm bg-slate-700 px-2 py-1 rounded text-slate-300">default (Shopify DTC)</button>
                </div>
                <div className="text-sm text-slate-300 mb-2">Data flows</div>
                <div className="text-sm text-slate-300 mb-2">Settings</div>
                
                <div className="text-sm text-slate-400 mt-6 mb-4">Mappings</div>
                <div className="text-sm text-blue-400 mb-2">Orders</div>
                <div className="text-sm text-slate-300 mb-2">Products</div>
                <div className="text-sm text-slate-300 mb-2">Fulfillments</div>
                <div className="text-sm text-slate-300 mb-2">Payouts</div>
                <div className="text-sm text-slate-300 mb-2">Refunds/Returns</div>
              </nav>
              
              <div className="p-4 border-t border-slate-700">
                <button className="w-full bg-blue-600 text-white px-3 py-2 rounded text-sm">+ Add connector</button>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="bg-white border-b border-slate-200 px-6 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h1 className="text-2xl font-bold text-slate-800">Shopify</h1>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-sm text-slate-600">Pirani Life, Inc.</span>
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-slate-200 rounded-full"></div>
                      <span className="text-sm text-slate-600">Scott Blubaugh</span>
                    </div>
              ...
              <div className="bg-white border-b border-slate-200 px-6">
                <nav className="-mb-px flex space-x-8">
                  <button className="border-b-2 border-blue-600 text-blue-600 px-1 py-4 text-sm font-medium">
                    Payment
                  </button>
                  <button className="border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 px-1 py-4 text-sm font-medium">
                    Shipment
                  </button>
                  <button className="border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 px-1 py-4 text-sm font-medium">
                    Order
                ...
                <div className="max-w-4xl">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-2 mb-2">
                      <label className="text-sm font-medium text-slate-700">Default payment method to post when no match found</label>
                      <div className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center">
                        <span className="text-xs text-slate-500">!</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Select defaultValue="do_not_post">
                        <SelectTrigger className="w-64">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="do_not_post">Do Not Post</SelectItem>
                          <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                          <SelectItem value="zero_sales">$0 Sales</SelectItem>
                        </SelectContent>
                      </Select>
                  ...
                  <div className="border border-slate-200 rounded-lg overflow-hidden">
                    ...
                    <div className="divide-y divide-slate-200">
                      {/* Visa */}
                      <div className="px-4 py-3 grid grid-cols-2 gap-4 items-center">
                        <div className="text-sm">Visa</div>
                        <div className="flex items-center gap-2">
                          <Select defaultValue="shopify_dtc_payout">
                            ...
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="visa_deposit">Visa Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex items-center gap-2">
                          <Select defaultValue="shopify_dtc_payout">
                            ...
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="mastercard_deposit">Mastercard Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="px-4 py-3 grid grid-cols-2 gap-4 items-center">
                        ...
                        <div className="flex items-center gap-2">
                          <Select defaultValue="zero_sales">
                            <SelectTrigger className="w-full">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="unknown_deposit">Unknown Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {/* American Express */}
                      <div className="px-4 py-3 grid grid-cols-2 gap-4 items-center">
                        <div className="text-sm">American Express</div>
                        <div className="flex items-center gap-2">
                          <Select defaultValue="shopify_dtc_payout">
                            <SelectTrigger className="w-full">
                              <SelectValue />
                            ...
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="amex_deposit">Amex Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex items-center gap-2">
                          <Select defaultValue="shopify_dtc_payout">
                            ...
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="discover_deposit">Discover Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {/* Shopify Payments */}
                      <div className="px-4 py-3 grid grid-cols-2 gap-4 items-center">
                        <div className="text-sm">shopify_payments</div>
                        <div className="flex items-center gap-2">
                          <Select defaultValue="shopify_dtc_payout">
                            <SelectTrigger className="w-full">
                              <SelectValue />
                            ...
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="shopify_payments_deposit">Shopify Payments Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {/* Blank */}
                      <div className="px-4 py-3 grid grid-cols-2 gap-4 items-center">
                        <div className="text-sm text-slate-400">(blank)</div>
                        <div className="flex items-center gap-2">
                          <Select defaultValue="zero_sales">
                            <SelectTrigger className="w-full">
                              <SelectValue />
                            ...
                            <SelectContent>
                              <SelectItem value="shopify_dtc_payout">Shopify DTC Payout</SelectItem>
                              <SelectItem value="zero_sales">$0 Sales</SelectItem>
                              <SelectItem value="blank_deposit">Blank Deposit</SelectItem>
                            </SelectContent>
                          </Select>
                          <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      case 'settings':
            <div className="border-b border-slate-200">
              <nav className="-mb-px flex space-x-8">
                <button className="border-b-2 border-blue-600 text-blue-600 px-1 py-2 text-sm font-medium">
                  Payment
                </button>
                <button className="border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 px-1 py-2 text-sm font-medium">
                  Shipment
                </button>
                <button className="border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 px-1 py-2 text-sm font-medium">
                  Order
            ...
            {/* Fulfilments Sub-tabs */}
            <div className="border-b border-slate-100">
              <nav className="-mb-px flex space-x-6 ml-4">
                <button className="border-b-2 border-blue-600 text-blue-600 px-1 py-2 text-sm font-medium">
                  Payouts
                </button>
                <button className="border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 px-1 py-2 text-sm font-medium">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <label className="text-sm font-medium">Default payout method to post when no match found</label>
                      <div className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center">
                        <span className="text-xs text-slate-500">!</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Select defaultValue="do_not_post">
                        ...
                        <SelectContent>
                          <SelectItem value="do_not_post">Do Not Post</SelectItem>
                          <SelectItem value="shopify_deposit">Shopify Deposit</SelectItem>
                          <SelectItem value="zero_sales">$0 Sales</SelectItem>
                        </SelectContent>
                      </Select>
                      <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold">Payout mappings</h3>
                      <div className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center">
                        <span className="text-xs text-slate-500">!</span>
                      </div>
                    </div>
                    ...
                    <div className="border border-slate-200 rounded-lg overflow-hidden">
                      <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
                        <div className="grid grid-cols-3 gap-4 text-sm font-medium text-slate-700">
                          <div>Shopify Payout Field</div>
                          <div className="flex items-center gap-2">
                            NetSuite Deposit Field
                            <div className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center">
                              <span className="text-xs text-slate-500">!</span>
                            </div>
                          </div>
                      ...
                      <div className="divide-y divide-slate-200">
                        ...
                        <div className="px-4 py-3 grid grid-cols-3 gap-4 items-center">
                          <div className="text-sm">Payout ID</div>
                          <div className="flex items-center gap-2">
                            <Select defaultValue="deposit_id">
                              <SelectTrigger className="w-full">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="deposit_id">Deposit ID</SelectItem>
                                <SelectItem value="external_id">External ID</SelectItem>
                                <SelectItem value="memo">Memo</SelectItem>
                              </SelectContent>
                            </Select>
                            <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor DTC Payout</SelectItem>
                                <SelectItem value="zero_sales">$0 Sales</SelectItem>
                                <SelectItem value="mastercard_deposit">Mastercard Deposit</SelectItem>
                              </SelectContent>
                            </Select>
                          ...
                          <div className="flex items-center gap-2">
                            <Select defaultValue="currency">
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="base_currency">Base Currency</SelectItem>
                              </SelectContent>
                            </Select>
                            <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <div className="flex items-center gap-2">
                            <Select defaultValue="status">
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="processing_status">Processing Status</SelectItem>
                              </SelectContent>
                            </Select>
                            <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="px-4 py-3 grid grid-cols-3 gap-4 items-center">
                          ...
                          <div className="flex items-center gap-2">
                            <Select defaultValue="deposit_date">
                              <SelectTrigger className="w-full min-w-[200px]">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                ...
                                <SelectItem value="processed_date">Processed Date</SelectItem>
                              </SelectContent>
                            </Select>
                            <Button variant="ghost" size="sm" className="w-8 h-8 p-0">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
