...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('❌ Error checking edited order state:', error)
...
/**
 * POST — Execute a processing step (or all steps) for an edited order.
 *
 * Body: {
 *   orderName: string,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`✏️ Process edited order: ${orderName}, step: ${step}`)
    ...
    if (step === 'auto') {
      ...
      const { results, finalState } = await processEditedOrder(
    ...
    switch (step) {
      ...
      case 'update-sales-order': {
        ...
        const result = await updateSOForEditedOrder(salesOrderId)
        return NextResponse.json({ success: result.success, result })
      }
    ...
    console.error('❌ Error processing edited order:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
